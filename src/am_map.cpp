...
const char* LevelNames[] = {
	...
	"E1M1:  THE DOCKS", "E1M2:  THE DUNGEONS", "E1M3:  THE GATEHOUSE", "E1M4:  THE GUARD TOWER", "E1M5:  THE CITADEL",
	"E1M6:  THE CATHEDRAL", "E1M7:  THE CRYPTS", "E1M8:  HELL'S MAW", "E1M9:  THE GRAVEYARD",
	/* EPISODE 2 - HELL'S MAW */
	"E2M1:  THE CRATER", "E2M2:  THE LAVA PITS", "E2M3:  THE RIVER OF FIRE", "E2M4:  THE ICE GROTTO",
	"E2M5:  THE CATACOMBS", "E2M6:  THE LABYRINTH", "E2M7:  THE GREAT HALL", "E2M8:  THE PORTALS OF CHAOS",
	"E2M9:  THE GLACIER",
	/* EPISODE 3 - THE DOME OF D'SPARIL */
	"E3M1:  THE STOREHOUSE", "E3M2:  THE CESSPOOL", "E3M3:  THE CONFLUENCE", "E3M4:  THE AZURE FORTRESS",
	"E3M5:  THE OPHIDIAN LAIR", "E3M6:  THE HALLS OF FEAR", "E3M7:  THE CHASM", "E3M8:  D'SPARIL'S KEEP",
	"E3M9:  THE AQUIFER",
	/* EPISODE 4: THE OSSUARY */
	"E4M1:  CATAFALQUE", "E4M2:  BLOCKHOUSE", "E4M3:  AMBULATORY", "E4M4:  SEPULCHER", "E4M5:  GREAT STAIR",
	"E4M6:  HALLS OF THE APOSTATE", "E4M7:  RAMPARTS OF PERDITION", "E4M8:  SHATTERED BRIDGE", "E4M9:  MAUSOLEUM",
	/* EPISODE 5: THE STAGNANT DEMESNE */
	"E5M1:  OCHRE CLIFFS", "E5M2:  RAPIDS", "E5M3:  QUAY", "E5M4:  COURTYARD", "E5M5:  HYDRATYR", "E5M6:  COLONNADE",
	"E5M7:  FOETID MANSE", "E5M8:  FIELD OF JUDGEMENT", "E5M9:  SKEIN OF D'SPARIL"};
...
static byte antialias[NUMALIAS][8] = {
	{96, 97, 98, 99, 100, 101, 102, 103}, {110, 109, 108, 107, 106, 105, 104, 103}, {75, 76, 77, 78, 79, 80, 81, 103}};
/*
...
void DrawWuLine(int X0, int Y0, int X1, int Y1, byte* BaseColor, int NumLevels, unsigned short IntensityBits);
...
void AM_initVariables(void) {
	...
	if (gameskill == sk_baby) {
		for (think = thinkercap.next; think != &thinkercap; think = think->next) {
			if (think->function.acp1 != (actionf_p1) P_MobjThinker) { /* not a mobj */
				continue;
			}
			mo = (mobj_t*) think;
			if (mo->type == MT_CKEY) {
...
boolean AM_Responder(event_t* ev) {
	...
	rc = false;
	if (!automapactive) {
		if (ev->type == ev_keydown && ev->data1 == AM_STARTKEY && gamestate == GS_LEVEL) {
			AM_Start();
			viewactive = false;
			/*      viewactive = true; */
			rc = true;
		...
		switch (ev->data1) {
			...
			case AM_FOLLOWKEY:
				...
				f_oldloc.x = MAXINT;
				P_SetMessage(
					plr, followplayer ? const_cast<char*>(AMSTR_FOLLOWON) : const_cast<char*>(AMSTR_FOLLOWOFF), true);
				break;
				/*
	    case AM_GRIDKEY:
	    grid = !grid;
...
void AM_clearFB([[maybe_unused]] int color) {
#if 0
  int i, j;
  int dmapx;
  int dmapy;
  ...
  if(followplayer)
	...
	if (screenwidth != SCREENWIDTH || SCREENHEIGHT != screenheight) {
		...
		for (y = 0; y < screenheight - SBARHEIGHT; y++) {
			for (x = 0; x + SCREENWIDTH <= screenwidth; x += SCREENWIDTH) {
...
void AM_drawFline(fline_t* fl, int color) {
	...
	switch (color) {
		case WALLCOLORS: DrawWuLine(fl->a.x, fl->a.y, fl->b.x, fl->b.y, &antialias[0][0], 8, 3); break;
		case FDWALLCOLORS: DrawWuLine(fl->a.x, fl->a.y, fl->b.x, fl->b.y, &antialias[1][0], 8, 3); break;
		case CDWALLCOLORS: DrawWuLine(fl->a.x, fl->a.y, fl->b.x, fl->b.y, &antialias[2][0], 8, 3); break;
		default: {
			/* For debugging only */

...
void* Z_Malloc(size_t size, int tag, void* user) {
	size_t extra;
	memblock_t *start, *rover, *new_mem, *base;
   ...
   */
	...
	if (extra > MINFRAGMENT) { /* there will be a free fragment after the allocated block */
		new_mem = (memblock_t*) ((byte*) base + size);
		new_mem->size = extra;
		new_mem->user = NULL; /* free block */
		new_mem->tag = 0;
		new_mem->prev = base;
		new_mem->next = base->next;
	...
	if (user) {
		base->user = reinterpret_cast<void**>(user); /* mark as an in use block */
		*(void**) user = (void*) ((byte*) base + sizeof(memblock_t));
	} else {
		if (tag >= PU_PURGELEVEL)
			I_Error("Z_Malloc: an owner is required for purgable blocks");
		base->user = reinterpret_cast<void**>(2); /* mark as in use, but unowned */
	}
	base->tag = tag;

...
static fixed_t FloatBobOffsets[64] = {0, 51389, 102283, 152192, 200636, 247147, 291278, 332604, 370727, 405280, 435929,
	462380, 484378, 501712, 514213, 521763, 524287, 521763, 514213, 501712, 484378, 462380, 435929, 405280, 370727,
	332604, 291278, 247147, 200636, 152192, 102283, 51389, -1, -51390, -102284, -152193, -200637, -247148, -291279,
...
boolean P_SetMobjState(mobj_t* mobj, statenum_t state) {
	...
	mobj->frame = st->frame;
	if (st->action.acp1) { /* Call action function */
		st->action.acp1(mobj);
	}
	return (true);
}
...
boolean P_SeekerMissile(mobj_t* actor, angle_t thresh, angle_t turnMax) {
	...
	if (actor->z + actor->height < target->z || target->z + target->height < actor->z) { /* Need to seek vertically */
...
void P_XYMovement(mobj_t* mo) {
	...
	if (mo->flags2 & MF2_WINDTHRUST) {
		switch (special) {
			case 40:
			case 41:
			case 42: /* Wind_East */ P_ThrustMobj(mo, 0, windTab[special - 40]); break;
			case 43:
			case 44:
			case 45: /* Wind_North */ P_ThrustMobj(mo, ANG90, windTab[special - 43]); break;
			case 46:
			case 47:
			case 48: /* Wind_South */ P_ThrustMobj(mo, ANG270, windTab[special - 46]); break;
			case 49:
			case 50:
			case 51: /* Wind_West */ P_ThrustMobj(mo, ANG180, windTab[special - 49]); break;
		}
	}
	player = mo->player;
	if (mo->momx > MAXMOVE) {
	...
	if (player && player->cheats & CF_NOMOMENTUM) { /* Debug option for no sliding at all */
		mo->momx = mo->momy = 0;
		return;
	}
	...
	if (mo->z > mo->floorz && !(mo->flags2 & MF2_FLY) && !(mo->flags2 & MF2_ONMOBJ)) { /* No friction when falling */
		return;
	}
	if (mo->flags & MF_CORPSE) { /* Don't stop sliding if halfway off a step with some momentum */
	...
	if (mo->momx > -STOPSPEED && mo->momx < STOPSPEED && mo->momy > -STOPSPEED && mo->momy < STOPSPEED &&
		(!player ||
			(player->cmd.forwardmove == 0 && player->cmd.sidemove == 0))) { /* If in a walking frame, stop moving */
		if (player) {
	...
	if (mo->z <= mo->floorz) { /* Hit the floor */
		if (mo->flags & MF_MISSILE) {
			...
			if (mo->flags2 & MF2_FLOORBOUNCE) {
				...
				return;
			} else if (mo->type == MT_MNTRFX2) { /* Minotaur floor fire can go up steps */
				return;
			} else {
				P_ExplodeMissile(mo);
				return;
...
void P_BlasterMobjThinker(mobj_t* mobj) {
	...
	if (mobj->momx || mobj->momy || (mobj->z != mobj->floorz) || mobj->momz) {
		...
		for (i = 0; i < 8; i++) {
			if (changexy) {
				if (!P_TryMove(mobj, mobj->x + xfrac, mobj->y + yfrac)) { /* Blocked move */
...
void P_MobjThinker(mobj_t* mobj) {
	...
	if (mobj->momx || mobj->momy || (mobj->flags & MF_SKULLFLY)) {
		P_XYMovement(mobj);
		if (mobj->thinker.function.acv == (actionf_v)(-1)) { /* mobj was removed */
			return;
		}
	}
	if (mobj->flags2 & MF2_FLOATBOB) { /* Floating item bobbing motion */
		mobj->z = mobj->floorz + FloatBobOffsets[(mobj->health++) & 63];
	} else if ((mobj->z != mobj->floorz) || mobj->momz) { /* Handle Z momentum and gravity */
...
void P_SpawnMapThing(mapthing_t* mthing) {
	...
	switch (i) { /* Special stuff */
		...
		case MT_ITEMSHIELD2:
			if (shareware) { /* Don't place on map in shareware version */
				return;
			}
			break;
		case MT_WMACE:
			if (!shareware) { /* Put in the mace spot list */
	...
	if (mobj->flags2 & MF2_FLOATBOB) { /* Seed random starting index for bobbing motion */
...
void P_SpawnPuff(fixed_t x, fixed_t y, fixed_t z) {
	...
	switch (PuffType) {
		...
		case MT_GAUNTLETPUFF2: puff->momz = .8 * FRACUNIT;
		default: break;
	}
}
...
int P_HitFloor(mobj_t* thing) {
	...
	if (thing->floorz !=
		thing->subsector->sector->floorheight) { /* don't splash if landing on the edge above water/lava/etc.... */
		return (FLOOR_SOLID);
	}
...
mobj_t* P_SpawnMissile(mobj_t* source, mobj_t* dest, mobjtype_t type) {
	...
	switch (type) {
		case MT_MNTRFX1: /* Minotaur swing attack missile */ z = source->z + 40 * FRACUNIT; break;
		case MT_MNTRFX2: /* Minotaur floor fire missile */ z = ONFLOORZ; break;
		case MT_SRCRFX1: /* Sorcerer Demon fireball */ z = source->z + 48 * FRACUNIT; break;
		case MT_KNIGHTAXE: /* Knight normal axe */
		case MT_REDAXE: /* Knight red power axe */ z = source->z + 36 * FRACUNIT; break;
...
mobj_t* P_SpawnPlayerMissile(mobj_t* source, mobjtype_t type) {
	...
	if (MissileMobj->type == MT_BLASTERFX1) { /* Ultra-fast ripper spawning missile */
	...
	if (!P_TryMove(MissileMobj, MissileMobj->x, MissileMobj->y)) { /* Exploded immediately */

...
void P_GroupLines(void) {
	...
	linebuffer = static_cast<line_t**>(Z_Malloc(total * sizeof(void*), PU_LEVEL, 0));
	sector = sectors;
...
void P_SetupLevel(int episode, int map, [[maybe_unused]] int playermask, [[maybe_unused]] skill_t skill) {
	int i;
	int parm;
	char lumpname[9];
	int lumpnum;
	...
	if (deathmatch) {
		for (i = 0; i < MAXPLAYERS; i++) {
			if (playeringame[i]) { /* must give a player spot before deathmatchspawn */
				mobj = P_SpawnMobj(playerstarts[i].x << 16, playerstarts[i].y << 16, 0, MT_PLAYER);
				players[i].mo = mobj;
...
void P_Init(void) {
	...
	P_InitTerrainTypes();
	P_InitLava();
	R_InitSprites(const_cast<char**>(sprnames), NUMSPRITES);
	printf("P_Init: Init switch list, pic anims, terrain types, lava, sprites complete.\n");
}

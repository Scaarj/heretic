...
struct {
	...
	{MT_IMPBALL, {10, 20}}, {MT_MUMMYFX1, {9, 18}}, {MT_KNIGHTAXE, {9, 18}}, {MT_REDAXE, {9, 18}},
	{MT_BEASTBALL, {12, 20}}, {MT_WIZFX1, {18, 24}}, {MT_SNAKEPRO_A, {14, 20}}, {MT_SNAKEPRO_B, {14, 20}},
	{MT_HEADFX1, {13, 20}}, {MT_HEADFX3, {10, 18}}, {MT_MNTRFX1, {20, 26}}, {MT_MNTRFX2, {14, 20}},
	{MT_SRCRFX1, {20, 28}}, {MT_SOR2FX1, {20, 28}}, {static_cast<mobjtype_t>(-1), {-1, -1}} /* Terminator */
};
...
boolean G_Responder(event_t* ev) {
	...
	if (ev->type == ev_keyup && ev->data1 == key_useartifact) { /* flag to denote that it's okay to use an artifact */
		if (!inventory) {
...
void G_PlayerFinishLevel(int player) {
  ...
  */
	...
	if (p->chickenTics) {
		p->readyweapon = static_cast<weapontype_t>(p->mo->special1); /* Restore weapon */
		p->chickenTics = 0;
	}
	p->messageTics = 0;
	p->lookdir = 0;
...
void G_LoadGame(char* name) {
	...
	// NOTE: more safe
	// strcpy(savename, name);
	strncpy(savename, name, sizeof(savename) / sizeof(char));
...
void G_DoLoadGame(void) {
	...
	if (strcmp(reinterpret_cast<char*>(save_p), vcheck)) { /* Bad version */
		return;
	}
	save_p += VERSIONSIZE;
	...
	if (*save_p != SAVE_GAME_TERMINATOR) { /* Missing savegame termination marker */
...
void G_WriteDemoTiccmd(ticcmd_t* cmd) {
	if (gamekeydown[static_cast<int>('q')]) /* press q to end demo recording */
		G_CheckDemoStatus();
...
void G_RecordDemo(skill_t skill, [[maybe_unused]] int numplayers, int episode, int map, char* name) {
	int i;

	G_InitNew(skill, episode, map);
	usergame = false;
	// NOTE: more safe
	strncpy(demoname, name, sizeof(demoname) / sizeof(char));
	strncat(demoname, ".lmp", sizeof(demoname) / sizeof(char));
	//	strcpy(demoname, name);
	//	strcat(demoname, ".lmp");
	demobuffer = demo_p = static_cast<byte*>(Z_Malloc(0x20000, PU_STATIC, NULL));
	*demo_p++ = skill;
	*demo_p++ = episode;
	*demo_p++ = map;
...
void G_SaveGame(int slot, char* description) {
	...
	strncpy(savedescription, description, sizeof(savedescription) / sizeof(char));
	sendsave = true;
}
...
void SV_Open(char* fileName) {
	...
	save_p = savebuffer = static_cast<byte*>(Z_Malloc(SAVEGAMESIZE, PU_STATIC, NULL));
	MallocFailureOk = false;
	if (savebuffer == NULL) { /* Not enough memory - use file save method */

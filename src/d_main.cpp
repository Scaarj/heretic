...
void D_Display(void) {
   ...
   */
	...
	if (paused && !MenuActive && !askforquit) {
		if (!netgame) {
			V_DrawPatch(160, viewwindowy + 5, static_cast<patch_t*>(W_CacheLumpName("PAUSED", PU_CACHE)));
		} else {
			V_DrawPatch(160, 70 + Y_DISP, static_cast<patch_t*>(W_CacheLumpName("PAUSED", PU_CACHE)));
		}
	}
...
void D_PageDrawer(void) {
	V_DrawRawScreen(static_cast<byte*>(W_CacheLumpName(pagename, PU_CACHE)));
	if (demosequence == 1) {
		V_DrawPatch(4, Y_DISP + 160, reinterpret_cast<patch_t*>(W_CacheLumpName("ADVISOR", PU_CACHE)));
	}
...
char* wadfiles[MAXWADFILES] = {"/Novell/Heretic/source/heretic.wad", "/Novell/Heretic/data/texture1.lmp",
	"/Novell/Heretic/data/texture2.lmp", "/Novell/Heretic/data/pnames.lmp"};
...
// TODO: Make variable
	#define SHAREWAREWADNAME "/usr/share/ru.auroraos.heretic/Heretic.wad"

const char* wadfiles[MAXWADFILES] = {"heretic.wad"
	#ifdef EXTRA_WADS
	,
	"texture1.lmp", "texture2.lmp", "pnames.lmp"
	#endif
};
...
void D_AddFile(char* file) {
	...
	for (numwadfiles = 0; wadfiles[numwadfiles]; numwadfiles++)
		;
	newf = static_cast<char*>(malloc(strlen(file) + 1));
	assert(newf);
	strncpy(newf, file, strlen(file) + 1);
	if (strlen(exrnwads) + strlen(file) < 78) {
		if (strlen(exrnwads)) {
			strncat(exrnwads, ", ", sizeof(exrnwads) / sizeof(char));
		} else {
			strncpy(exrnwads, "External Wadfiles: ", sizeof(exrnwads) / sizeof(char));
		}
		strncat(exrnwads, file, sizeof(exrnwads) / sizeof(char));
	} else if (strlen(exrnwads2) + strlen(file) < 79) {
		if (strlen(exrnwads2)) {
			strncat(exrnwads2, ", ", sizeof(exrnwads2) / sizeof(char));
		} else {
			strcpy(exrnwads2, "     ");
			strncat(exrnwads, ",", sizeof(exrnwads) / sizeof(char));
		}
		strncat(exrnwads2, file, sizeof(exrnwads2) / sizeof(char));
	}
...
void D_DoomMain(void) {
	...
	if (getenv("PATH")) {
		...
		char* newpath = static_cast<char*>(malloc(strlen(path) + 2));
	...
	if (M_CheckParm("-linear") != 0)
		lifilter = 1;
	if (M_CheckParm("-bilinear") != 0)
		bilifilter = 1;
   ...
   */
	...
	else { /* Change to look for shareware wad */
   ...
   */
	...
	if (W_CheckNumForName("E2M1") == -1) { /* Can't find episode 2 maps, must be the shareware WAD */
		shareware = true;
	} else if (W_CheckNumForName("EXTENDED") != -1) { /* Found extended lump, must be the extended WAD */
		ExtendedWAD = true;
	}

...
static weapontype_t GetAmmoChange[] = {wp_goldwand, wp_crossbow, wp_blaster, wp_skullrod, wp_phoenixrod, wp_mace};
...
boolean P_GiveAmmo(player_t* player, ammotype_t ammo, int count) {
	...
	if (gameskill == sk_baby || gameskill == sk_nightmare) { /* extra ammo in baby mode and nightmare mode */
		count += count >> 1;
	}
...
boolean P_GiveWeapon(player_t* player, weapontype_t weapon) {
	...
	if (player->weaponowned[weapon]) {
		...
		gaveWeapon = true;
		player->weaponowned[weapon] = true;
		if (WeaponValue[weapon] > WeaponValue[player->readyweapon]) { /* Only switch to more powerful weapons */
			player->pendingweapon = weapon;
		}
	}
...
boolean P_GiveArtifact(player_t* player, artitype_t arti, mobj_t* mo) {
	...
	if (i == player->inventorySlotNum) {
		...
		if (player->inventory[i].count >= 16) { /* Player already has 16 of this item */
			return (false);
		}
...
void P_SetDormantArtifact(mobj_t* arti) {
	...
	if (deathmatch && (arti->type != MT_ARTIINVULNERABILITY) && (arti->type != MT_ARTIINVISIBILITY)) {
...
void P_KillMobj(mobj_t* source, mobj_t* target) {
	...
	} else if (!netgame && (target->flags & MF_COUNTKILL)) { /* Count all monster deaths */
		players[0].killcount++;
	}
	if (target->player) {
		if (!source) { /* Self-frag */
	...
	if (target->health < -(target->info->spawnhealth >> 1) && target->info->xdeathstate) { /* Extreme death */
...
void P_AutoUseHealth(player_t* player, int saveHealth) {
	...
	if ((gameskill == sk_baby) && (normalCount * 25 >= saveHealth)) { /* Use quartz flasks */
		...
		(superCount * 100 + normalCount * 25 >= saveHealth)) { /* Use mystic urns and quartz flasks */
		count = (saveHealth + 24) / 25;
		saveHealth -= count * 25;
		for (i = 0; i < count; i++) {
			player->health += 25;
...
void P_DamageMobj(mobj_t* target, mobj_t* inflictor, mobj_t* source, int damage) {
	...
	if (target->flags & MF_SKULLFLY) {
		if (target->type == MT_MINOTAUR) { /* Minotaur is invulnerable during charge attack */
			return;
		}
	...
	if (inflictor) {
		switch (inflictor->type) {
			...
			case MT_MACEFX4: /* Death ball */
				if ((target->flags2 & MF2_BOSS) || target->type == MT_HEAD) { /* Don't allow cheap boss kills */
					break;
				} else if (target->player) { /* Player specific checks */
					if (target->player->powers[pw_invulnerability]) { /* Can't hurt invulnerable players */
						break;
					}
					if (P_AutoUseChaosDevice(target->player)) { /* Player was saved using chaos device */
						return;
					}
				}
			...
			case MT_HORNRODFX2:
			case MT_PHOENIXFX1:
				if (target->type == MT_SORCERER2 && P_Random() < 96) { /* D'Sparil teleports away */
	...
	if (inflictor && (!source || !source->player || source->player->readyweapon != wp_gauntlets) &&
		...
		if (source && source->player && (source == inflictor) && source->player->powers[pw_weaponlevel2] &&
   ...
   */
	...
	if (target->health <= 0) { /* Death */
		...
		if (target->type == MT_POD && source &&
			source->type != MT_POD) { /* Make sure players get frags for chain-reaction kills */
			target->target = source;
		}
		if (player && inflictor && !player->chickenTics) { /* Check for flame death */
			if ((inflictor->flags2 & MF2_FIREDAMAGE) ||
				((inflictor->type == MT_PHOENIXFX1) && (target->health > -50) && (damage > 25))) {

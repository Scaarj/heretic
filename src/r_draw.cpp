...
void R_DrawViewBorder(void) {
	...
	if (shareware) {
		src = static_cast<byte*>(W_CacheLumpName("FLOOR04", PU_CACHE));
	} else {
		src = static_cast<byte*>(W_CacheLumpName("FLAT513", PU_CACHE));
	}
	dest = screen;
	...
	for (x = viewwindowx; x < viewwindowx + viewwidth; x += 16) {
		V_DrawPatch(x - X_DISP, viewwindowy - 4, static_cast<patch_t*>(W_CacheLumpName("bordt", PU_CACHE)));
		V_DrawPatch(x - X_DISP, viewwindowy + viewheight, static_cast<patch_t*>(W_CacheLumpName("bordb", PU_CACHE)));
	}
	for (y = viewwindowy; y < viewwindowy + viewheight; y += 16) {
		V_DrawPatch(viewwindowx - 4 - X_DISP, y, static_cast<patch_t*>(W_CacheLumpName("bordl", PU_CACHE)));
		V_DrawPatch(viewwindowx + viewwidth - X_DISP, y, static_cast<patch_t*>(W_CacheLumpName("bordr", PU_CACHE)));
	}
	V_DrawPatch(viewwindowx - 4 - X_DISP, viewwindowy - 4, static_cast<patch_t*>(W_CacheLumpName("bordtl", PU_CACHE)));
	V_DrawPatch(
		viewwindowx + viewwidth - X_DISP, viewwindowy - 4, static_cast<patch_t*>(W_CacheLumpName("bordtr", PU_CACHE)));
	V_DrawPatch(viewwindowx + viewwidth - X_DISP, viewwindowy + viewheight,
		static_cast<patch_t*>(W_CacheLumpName("bordbr", PU_CACHE)));
	V_DrawPatch(
		viewwindowx - 4 - X_DISP, viewwindowy + viewheight, static_cast<patch_t*>(W_CacheLumpName("bordbl", PU_CACHE)));
}
...
void R_DrawTopBorder(void) {
	...
	if (viewwindowy < 28) {
		...
		V_DrawPatch(viewwindowx - 4 - X_DISP, viewwindowy, static_cast<patch_t*>(W_CacheLumpName("bordl", PU_CACHE)));
		V_DrawPatch(
			viewwindowx + viewwidth - X_DISP, viewwindowy, static_cast<patch_t*>(W_CacheLumpName("bordr", PU_CACHE)));
		V_DrawPatch(
			viewwindowx - 4 - X_DISP, viewwindowy + 16, static_cast<patch_t*>(W_CacheLumpName("bordl", PU_CACHE)));
		V_DrawPatch(viewwindowx + viewwidth - X_DISP, viewwindowy + 16,
			static_cast<patch_t*>(W_CacheLumpName("bordr", PU_CACHE)));

		V_DrawPatch(
			viewwindowx - 4 - X_DISP, viewwindowy - 4, static_cast<patch_t*>(W_CacheLumpName("bordtl", PU_CACHE)));

...
const char* CT_FromPlrText[MAXPLAYERS] = {"GREEN:  ", "YELLOW:  ", "RED:  ", "BLUE:  "};
...
boolean CT_Responder(event_t* ev) {
	...
	if (!chatmodeon) {
		...
		if (ev->data1 == KEY_ENTER) {
			...
			if (ev->data1 == ' ' || ev->data1 == ',' || ev->data1 == '.' || (ev->data1 >= '0' && ev->data1 <= '9') ||
				ev->data1 == '\'' || ev->data1 == KEY_BACKSPACE || ev->data1 == '-' || ev->data1 == '=') {
	...
	for (i = 0; i < MAXPLAYERS; i++) {
		...
		if ((c = players[i].cmd.chatchar) != 0) {
			if (c <= 5) {
				...
				if (numplayers > 2) {
					// NOTE: more safe
					strncpy(plr_lastmsg[i], CT_FromPlrText[i], sizeof(plr_lastmsg) / sizeof(char));
					strncat(plr_lastmsg[i], chat_msg[i], sizeof(plr_lastmsg) / sizeof(char));
				} else {
					strncpy(plr_lastmsg[i], chat_msg[i], sizeof(plr_lastmsg) / sizeof(char));
				}
				if (i != consoleplayer && (chat_dest[i] == consoleplayer + 1 || chat_dest[i] == CT_PLR_ALL) &&
					*chat_msg[i]) {
					...
					if (numplayers > 1) {
						...
						P_SetMessage(&players[consoleplayer], "THERE ARE NO OTHER PLAYERS IN THE GAME!", true);
						S_StartSound(NULL, sfx_chat);
					}
				}
				CT_ClearChatMessage(i);
	...
	if (chatmodeon) {
		...
		for (i = 0; i < msgptr[consoleplayer]; i++) {
			if (chat_msg[consoleplayer][i] < 33) {
				...
				patch =
					static_cast<patch_t*>(W_CacheLumpNum(FontABaseLump + chat_msg[consoleplayer][i] - 33, PU_CACHE));
...
void CT_AddChar(int player, char c) {
	...
	if (msgptr[player] + 1 >= MESSAGESIZE || msglen[player] >= MESSAGELEN) { /* full. */
		return;
	}

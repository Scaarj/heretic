...
void T_MoveCeiling(ceiling_t* ceiling) {
	...
	switch (ceiling->direction) {
		...
		case -1: /* DOWN */
			res = T_MovePlane(
				ceiling->sector, ceiling->speed, ceiling->bottomheight, ceiling->crush, 1, ceiling->direction);
			if (!(leveltime & 7))
				S_StartSound((mobj_t*) &ceiling->sector->soundorg, sfx_dormov);
			if (res == pastdest)
				switch (ceiling->type) {
       ...
       */
		...
		switch (type) {
			case fastCrushAndRaise:
				...
				break;
			case crushAndRaise: ceiling->crush = true; ceiling->topheight = sec->ceilingheight;
			case lowerAndCrush:
			case lowerToFloor:

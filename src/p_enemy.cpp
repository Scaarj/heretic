...
void P_RecursiveSound(sector_t* sec, int soundblocks) {
	...
	if (sec->validcount == validcount && sec->soundtraversed <= soundblocks + 1) { /* Already flooded */
		return;
	}
...
boolean P_CheckMissileRange(mobj_t* actor) {
	...
	dist = (P_AproxDistance(actor->x - actor->target->x, actor->y - actor->target->y) >> FRACBITS) - 64;
...
boolean P_LookForMonsters(mobj_t* actor) {
	...
	for (think = thinkercap.next; think != &thinkercap; think = think->next) {
		...
		mo = (mobj_t*) think;
		if (!(mo->flags & MF_COUNTKILL) || (mo == actor) || (mo->health <= 0)) { /* Not a valid monster */
			continue;
		}
		if (P_AproxDistance(actor->x - mo->x, actor->y - mo->y) > MONS_LOOK_RANGE) { /* Out of range */
			continue;
		}
		if (P_Random() < 16) { /* Skip */
			continue;
...
boolean P_LookForPlayers(mobj_t* actor, boolean allaround) {
	...
	if (!netgame && players[0].health <= 0) { /* Single player game and player is dead, look for monsters */
...
void A_Chase(mobj_t* actor) {
   ...
   */
	...
	if (!actor->target || !(actor->target->flags & MF_SHOOTABLE)) { /* look for a new target */
...
void A_DripBlood(mobj_t* actor) {
	...
	mo = P_SpawnMobj(
		actor->x + ((P_Random() - P_Random()) << 11), actor->y + ((P_Random() - P_Random()) << 11), actor->z, MT_BLOOD);
...
void A_ImpExplode(mobj_t* actor) {
	...
	mo->momy = (P_Random() - P_Random()) << 10;
	mo->momz = 9 * FRACUNIT;
	if (actor->special1 == 666) { /* Extreme death crash */
...
void A_BeastPuff(mobj_t* actor) {
	if (P_Random() > 64) {
		P_SpawnMobj(actor->x + ((P_Random() - P_Random()) << 10), actor->y + ((P_Random() - P_Random()) << 10),
...
boolean P_UpdateChicken(mobj_t* actor, int tics) {
	...
	moType = static_cast<mobjtype_t>(actor->special2);
	x = actor->x;
	y = actor->y;
	z = actor->z;
	oldChicken = *actor;
...
void A_Srcr1Attack(mobj_t* actor) {
	...
	if (actor->health > (actor->info->spawnhealth / 3) * 2) { /* Spit one fireball */
		...
		if (actor->health < actor->info->spawnhealth / 3) { /* Maybe attack again */
			if (actor->special1) { /* Just attacked, so don't attack again */
				actor->special1 = 0;
...
void A_Sor2DthLoop(mobj_t* actor) {
	if (--actor->special1) { /* Need to loop */
...
/*
  ...
  //----------------------------------------------------------------------------
*/
void A_SorZap([[maybe_unused]] mobj_t* actor) {
	S_StartSound(NULL, sfx_sorzap);
}
void A_SorRise([[maybe_unused]] mobj_t* actor) {
	S_StartSound(NULL, sfx_sorrise);
}
void A_SorDSph([[maybe_unused]] mobj_t* actor) {
	S_StartSound(NULL, sfx_sordsph);
}
void A_SorDExp([[maybe_unused]] mobj_t* actor) {
	S_StartSound(NULL, sfx_sordexp);
}
void A_SorDBon([[maybe_unused]] mobj_t* actor) {
	S_StartSound(NULL, sfx_sordbon);
}
void A_SorSightSnd([[maybe_unused]] mobj_t* actor) {
...
void A_MinotaurDecide(mobj_t* actor) {
	...
	if (target->z + target->height > actor->z && target->z + target->height < actor->z + actor->height &&
		dist < 8 * 64 * FRACUNIT && dist > 1 * 64 * FRACUNIT && P_Random() < 150) { /* Charge attack */
	...
	} else if (target->z == target->floorz && dist < 9 * 64 * FRACUNIT && P_Random() < 220) { /* Floor fire attack */
...
void A_MntrFloorFire(mobj_t* actor) {
	...
	actor->z = actor->floorz;
	mo = P_SpawnMobj(actor->x + ((P_Random() - P_Random()) << 10), actor->y + ((P_Random() - P_Random()) << 10),
		ONFLOORZ, MT_MNTRFX3);
	mo->target = actor->target;
...
void A_Scream(mobj_t* actor) {
	switch (actor->type) {
		...
		case MT_PLAYER:
			...
			if (actor->special1 < 10) { /* Wimpy death sound */
...
void A_Explode(mobj_t* actor) {
	...
	switch (actor->type) {
		case MT_FIREBOMB: /* Time Bombs */
			...
			actor->flags &= ~MF_SHADOW;
			break;
		case MT_MNTRFX2: /* Minotaur floor fire */ damage = 24; break;
		case MT_SOR2FX1: /* D'Sparil missile */ damage = 80 + (P_Random() & 31); break;
		default: break;
	}
...
void A_MakePod(mobj_t* actor) {
	...
	if (actor->special1 == MAX_GEN_PODS) { /* Too many generated pods */
		return;
	}
	x = actor->x;
	y = actor->y;
...
void A_BossDeath(mobj_t* actor) {
	...
	thinker_t* think;
	line_t dummyLine;
	static mobjtype_t bossType[6] = {static_cast<mobjtype_t>(MT_HEAD), static_cast<mobjtype_t>(MT_MINOTAUR),
		static_cast<mobjtype_t>(MT_SORCERER2), static_cast<mobjtype_t>(MT_HEAD), static_cast<mobjtype_t>(MT_MINOTAUR),
		static_cast<mobjtype_t>(-1)};

	if (gamemap != 8) { /* Not a boss level */
		return;
	}
	if (actor->type != bossType[gameepisode - 1]) { /* Not considered a boss in this episode */
		return;
	}
	...
	for (think = thinkercap.next; think != &thinkercap; think = think->next) {
		...
		mo = (mobj_t*) think;
		if ((mo != actor) && (mo->type == actor->type) && (mo->health > 0)) { /* Found a living boss */
			return;
		}
	}
...
void A_SpawnTeleGlitter(mobj_t* actor) {
	...
	mo = P_SpawnMobj(actor->x + ((P_Random() & 31) - 16) * FRACUNIT, actor->y + ((P_Random() & 31) - 16) * FRACUNIT,
...
void A_AddPlayerCorpse(mobj_t* actor) {
	if (bodyqueslot >= BODYQUESIZE) { /* Too many player corpses - remove an old one */

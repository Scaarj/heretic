...
boolean M_ValidEpisodeMap(int episode, int map) {
	...
	if (shareware) { /* Shareware version checks */
		if (episode != 1) {
			return false;
		}
	} else if (ExtendedWAD) { /* Extended version checks */
		if (episode == 6) {
			if (map > 3) {
				return false;
			}
...
unsigned char rndtable[256] = {0, 8, 109, 220, 222, 241, 149, 107, 75, 248, 254, 140, 16, 66, 74, 21, 211, 47, 80, 242,
	154, 27, 205, 128, 161, 89, 77, 36, 95, 110, 85, 48, 212, 140, 211, 249, 22, 79, 200, 50, 28, 188, 52, 140, 202,
	120, 68, 145, 62, 70, 184, 190, 91, 197, 152, 224, 149, 104, 25, 178, 252, 182, 202, 182, 141, 197, 4, 81, 181, 242,
	145, 42, 39, 227, 156, 198, 225, 193, 219, 93, 122, 175, 249, 0, 175, 143, 70, 239, 46, 246, 163, 53, 163, 109, 168,
	135, 2, 235, 25, 92, 20, 145, 138, 77, 69, 166, 78, 176, 173, 212, 166, 113, 94, 161, 41, 50, 239, 49, 111, 164, 70,
	60, 2, 37, 171, 75, 136, 156, 11, 56, 42, 146, 138, 229, 73, 146, 77, 61, 98, 196, 135, 106, 63, 197, 195, 86, 96,
	203, 113, 101, 170, 247, 181, 113, 80, 250, 108, 7, 255, 237, 129, 226, 79, 107, 112, 166, 103, 241, 24, 223, 239,
	120, 198, 58, 60, 82, 128, 3, 184, 66, 143, 224, 145, 224, 81, 206, 163, 45, 63, 90, 168, 114, 59, 33, 159, 95, 28,
	139, 123, 98, 125, 196, 15, 70, 194, 253, 54, 14, 109, 226, 71, 17, 161, 93, 186, 87, 244, 138, 20, 52, 123, 251,
	26, 36, 17, 46, 52, 231, 232, 76, 31, 221, 84, 37, 216, 165, 212, 106, 197, 242, 98, 43, 39, 175, 254, 145, 190, 84,
...
default_t defaults[] = {{"mouse_sensitivity", &mouseXSensitivity, 5, 0, 0},
	...
	{"chatmacro0", (long*) &chat_macros[0], (long) HUSTR_CHATMACRO0, 0, 0},
	{"chatmacro1", (long*) &chat_macros[1], (long) HUSTR_CHATMACRO1, 0, 0},
	{"chatmacro2", (long*) &chat_macros[2], (long) HUSTR_CHATMACRO2, 0, 0},
	{"chatmacro3", (long*) &chat_macros[3], (long) HUSTR_CHATMACRO3, 0, 0},
	{"chatmacro4", (long*) &chat_macros[4], (long) HUSTR_CHATMACRO4, 0, 0},
	{"chatmacro5", (long*) &chat_macros[5], (long) HUSTR_CHATMACRO5, 0, 0},
	{"chatmacro6", (long*) &chat_macros[6], (long) HUSTR_CHATMACRO6, 0, 0},
   ...
   */
	...
	if (f) {
		while (!feof(f)) {
			...
			if (fscanf(f, "%79s %[^\n]\n", def, strparm) == 2) {
				if (strparm[0] == '"') {
					...
					assert(newstring);
					strparm[len - 1] = 0;
					strncpy(newstring, strparm + 1, len);
				...
				for (i = 0; i < numdefaults; i++)
					if (!strcmp(def, defaults[i].name)) {
						if (!isstring)
							...
							*defaults[i].location = (long) newstring;
						break;
					}
			}
		}
		...
		if (screenblocks > maxblocks)
			screenblocks = maxblocks;
		if (screenblocks < minblocks)
			screenblocks = minblocks;
	}
}

...
boolean PIT_CheckLine(line_t* ld) {
  ...
  */
	...
	if (!(tmthing->flags & MF_MISSILE)) {
		...
		if (!tmthing->player && ld->flags & ML_BLOCKMONSTERS && tmthing->type != MT_POD) { /* Block monsters only */
			return (false);
		}
	}
...
boolean PIT_CheckThing(mobj_t* thing) {
	...
	if (!(thing->flags & (MF_SOLID | MF_SPECIAL | MF_SHOOTABLE))) { /* Can't hit thing */
		return (true);
	}
	blockdist = thing->radius + tmthing->radius;
	if (abs(thing->x - tmx) >= blockdist || abs(thing->y - tmy) >= blockdist) { /* Didn't hit thing */
		return (true);
	}
	if (thing == tmthing) { /* Don't clip against self */
		return (true);
	}
	if (tmthing->flags2 & MF2_PASSMOBJ) { /* check if a mobj passed over/under another object */
		if ((tmthing->type == MT_IMP || tmthing->type == MT_WIZARD) &&
			(thing->type == MT_IMP ||
				thing->type == MT_WIZARD)) { /* don't let imps/wizards fly over other imps/wizards */
			return false;
		}
		if (tmthing->z > thing->z + thing->height && !(thing->flags & MF_SPECIAL)) {
			return (true);
		} else if (tmthing->z + tmthing->height < thing->z && !(thing->flags & MF_SPECIAL)) { /* under thing */
			return (true);
		}
	}
	...
	if (tmthing->flags & MF_MISSILE) {
		...
		if (tmthing->target && tmthing->target->type == thing->type) { /* Don't hit same species as originator */
			if (thing == tmthing->target) { /* Don't missile self */
				return (true);
			}
			if (thing->type != MT_PLAYER) { /* Hit same species as originator, explode, no damage */
				return (false);
			}
		}
		...
		if (tmthing->flags2 & MF2_RIP) {
			...
			if (thing->flags2 & MF2_PUSHABLE && !(tmthing->flags2 & MF2_CANNOTPUSH)) { /* Push thing */
...
boolean P_TestMobjLocation(mobj_t* mobj) {
	...
	if (P_CheckPosition(mobj, mobj->x, mobj->y)) { /* XY is ok, now check Z */
		mobj->flags = flags;
		if ((mobj->z < mobj->floorz) || (mobj->z + mobj->height > mobj->ceilingz)) { /* Bad Z */
			return (false);
		}
		return (true);
	}
...
boolean P_TryMove(mobj_t* thing, fixed_t x, fixed_t y) {
	...
	if (!(thing->flags & MF_NOCLIP)) {
		...
		if (!(thing->flags & MF_TELEPORT)
			...
			&& thing->type != MT_MNTRFX2 && tmfloorz - thing->z > 24 * FRACUNIT) { /* Too big a step up */
			CheckMissileImpact(thing);
			return false;
		}
...
retry:
   ...
   * move up to the wall
   */
	if (bestslidefrac == FRACUNIT + 1) { /* the move most have hit the middle, so stairstep */
	stairstep:
...
boolean PIT_RadiusAttack(mobj_t* thing) {
	...
	if (thing->type == MT_MINOTAUR || thing->type == MT_SORCERER1 ||
		thing->type == MT_SORCERER2) { /* Episode 2 and 3 bosses take no damage from PIT_RadiusAttack */
		return (true);
	}

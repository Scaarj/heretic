...
void R_ClipSolidWallSegment(int first, int last) {
	...
	if (first < start->first) {
		if (last < start->first - 1) { /* post is entirely visible (above start), so insert a new clippost */
...
int checkcoord[12][4] = {{3, 0, 2, 1}, {3, 0, 2, 0}, {3, 1, 2, 0}, {0}, {2, 0, 2, 1}, {0, 0, 0, 0}, {3, 1, 3, 0}, {0},
...
void R_Subsector(int num) {
	...
	if (frontsector->floorheight < viewz)
		floorplane =
			R_FindPlane(frontsector->floorheight, frontsector->floorpic, frontsector->lightlevel, frontsector->special);
	else
		floorplane = NULL;
	if (frontsector->ceilingheight > viewz || frontsector->ceilingpic == skyflatnum)
		ceilingplane = R_FindPlane(frontsector->ceilingheight, frontsector->ceilingpic, frontsector->lightlevel, 0);
	else
		ceilingplane = NULL;

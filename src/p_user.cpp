...
void P_CalcHeight(player_t* player) {
   ...
   */
	...
	if (player->mo->flags2 & MF2_FEETARECLIPPED && player->playerstate != PST_DEAD &&
...
void P_DeathThink(player_t* player) {
	...
	if (player->attacker && player->attacker != player->mo) {
		...
		if (delta < ANG5 || delta > (unsigned) -ANG5) { /* Looking at killer, so fade damage flash down */
...
boolean P_UndoPlayerChicken(player_t* player) {
	...
	player->mo = mo;
	angle >>= ANGLETOFINESHIFT;
	fog = P_SpawnMobj(x + 20 * finecosine[angle], y + 20 * finesine[angle], z + TELEFOGHEIGHT, MT_TFOG);
...
void P_PlayerThink(player_t* player) {
	...
	cmd = &player->cmd;
	if (player->mo->flags & MF_JUSTATTACKED) { /* Gauntlets attack auto forward motion */
		cmd->angleturn = 0;
   ...
   */
	...
	if (cmd->buttons & BT_CHANGE) {
       ...
       */
		...
		if (newweapon == wp_staff && player->weaponowned[wp_gauntlets] && !(player->readyweapon == wp_gauntlets)) {
	...
	if (player->powers[pw_invulnerability]) {
		...
		if (player->powers[pw_infrared] <= BLINKTHRESHOLD) {
			...
			if (newtorch) {
				if (player->fixedcolormap + newtorchdelta > 7 || player->fixedcolormap + newtorchdelta < 1 ||
...
void P_PlayerRemoveArtifact(player_t* player, int slot) {
	...
	if (!(--player->inventory[slot].count)) { /* Used last of a type - compact the artifact list */
		...
		if (player == &players[consoleplayer]) { /* Set position markers and get next readyArtifact */
			inv_ptr--;
			if (inv_ptr < 6) {
				curpos--;
				if (curpos < 0) {
...
void P_PlayerUseArtifact(player_t* player, artitype_t arti) {
	...
	for (i = 0; i < player->inventorySlotNum; i++) {
		if (player->inventory[i].type == arti) { /* Found match - try to use */
			if (P_UseArtifact(player, arti)) { /* Artifact was used - remove it from inventory */
...
boolean P_UseArtifact(player_t* player, artitype_t arti) {
	...
	switch (arti) {
		...
		case arti_firebomb:
			...
			mo = P_SpawnMobj(player->mo->x + 24 * finecosine[angle], player->mo->y + 24 * finesine[angle],
				player->mo->z - 15 * FRACUNIT * ((player->mo->flags2 & MF2_FEETARECLIPPED) != 0), MT_FIREBOMB);
			mo->target = player->mo;
			break;
		case arti_egg:
			mo = player->mo;

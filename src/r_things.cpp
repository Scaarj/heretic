...
void R_InstallSpriteLump(int lump, unsigned frame, unsigned rotation, boolean flipped) {
	...
	if (sprtemp[frame].lump[rotation] != -1)
		I_Error(
			"R_InitSprites: Sprite %s : %c : %c has two lumps mapped to it", spritename, 'A' + frame, '1' + rotation);
       ...
       */
		...
		for (frame = 0; frame < maxframe; frame++) {
			switch ((int) sprtemp[frame].rotate) {
				...
				case 0: /* only the first rotation is needed */ break;
...
void R_ProjectSprite(mobj_t* thing) {
	...
	if (thing->flags2 & MF2_DONTDRAW) { /* Never make a vissprite when MF2_DONTDRAW is flagged. */
		return;
	}

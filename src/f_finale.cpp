...
boolean F_Responder(event_t* event) {
	...
	if (finalestage == 1 && gameepisode == 2) { /* we're showing the water pic, make any key kick to demo mode */
		finalestage++;
...
void F_DemonScroll(void) /* whether it would work ??? */
{
	...
	p1 = static_cast<byte*>(W_CacheLumpName("FINAL1", PU_LEVEL));
	p2 = static_cast<byte*>(W_CacheLumpName("FINAL2", PU_LEVEL));
	if (finalecount < 70) {
	...
	switch (finalestage) {
		case 1:
			if (!underwawa) {
				underwawa = true;
				memset(screen, 0, screenwidth * screenheight);
				I_SetPalette(static_cast<byte*>(W_CacheLumpName("E2PAL", PU_CACHE)));
...
void F_Drawer(void) {
	...
	if (!finalestage)
		...
		switch (gameepisode) {
			case 1:
				if (shareware) {
					V_DrawRawScreen(static_cast<byte*>(W_CacheLumpName("ORDER", PU_CACHE)));
				} else {
					V_DrawRawScreen(static_cast<byte*>(W_CacheLumpName("CREDIT", PU_CACHE)));
				}
				break;
			case 2: F_DrawUnderwater(); break;
			case 3: F_DemonScroll(); break;
			case 4: /* Just show credits screen for extended episodes */
			case 5: V_DrawRawScreen(static_cast<byte*>(W_CacheLumpName("CREDIT", PU_CACHE))); break;
		}
	}
}

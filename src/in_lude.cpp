...
void IN_InitStats(void) {
	...
	if (!netgame) {
		...
		if (playercount == slaughtercount) { /* don't do the slaughter stuff if everyone is equal */
			slaughterboy = 0;
		}
	}
}
...
void IN_LoadPics(void) {
	...
	switch (gameepisode) {
		case 1: patchINTERPIC = static_cast<patch_t*>(W_CacheLumpName("MAPE1", PU_STATIC)); break;
		case 2: patchINTERPIC = static_cast<patch_t*>(W_CacheLumpName("MAPE2", PU_STATIC)); break;
		case 3: patchINTERPIC = static_cast<patch_t*>(W_CacheLumpName("MAPE3", PU_STATIC)); break;
		default: break;
	}
	patchBEENTHERE = static_cast<patch_t*>(W_CacheLumpName("IN_X", PU_STATIC));
	patchGOINGTHERE = static_cast<patch_t*>(W_CacheLumpName("IN_YAH", PU_STATIC));
...
void IN_UnloadPics(void) {
	...
	if (patchINTERPIC) {
		Z_ChangeTag(reinterpret_cast<byte*>(patchINTERPIC), PU_CACHE);
	}
	Z_ChangeTag(reinterpret_cast<byte*>(patchBEENTHERE), PU_CACHE);
	Z_ChangeTag(reinterpret_cast<byte*>(patchGOINGTHERE), PU_CACHE);
	for (i = 0; i < 10; i++) {
		Z_ChangeTag(reinterpret_cast<byte*>(FontBNumbers[i]), PU_CACHE);
	}
	Z_ChangeTag(reinterpret_cast<byte*>(FontBNegative), PU_CACHE);
	Z_ChangeTag(reinterpret_cast<byte*>(FontBSlash), PU_CACHE);
	Z_ChangeTag(reinterpret_cast<byte*>(FontBPercent), PU_CACHE);
}
...
void IN_Ticker(void) {
	...
	if (oldintertime < intertime) {
		interstate++;
		if (gameepisode > 3 && interstate >= 1) { /* Extended Wad levels:  skip directly to the next level */
			interstate = 3;
		}
		switch (interstate) {
			case 0:
...
void IN_DrawStatBack(void) {
	...
	src = static_cast<byte*>(W_CacheLumpName("FLOOR16", PU_CACHE));
	dest = screen;
...
void IN_DrawOldLevel(void) {
	...
	if (prevmap == 9) {
		...
		if (!(intertime & 16)) {
			V_DrawPatch(
				YAHspot[gameepisode - 1][prevmap - 1].x, YAHspot[gameepisode - 1][prevmap - 1].y, patchBEENTHERE);
		}
	}
}
	...
	for (i = 0; i < MAXPLAYERS; i++) {
		if (playeringame[i]) {
			if (intertime < 100 || i == consoleplayer) {
				...
				V_DrawFuzzPatch(40, ypos, static_cast<patch_t*>(W_CacheLumpNum(patchFaceOkayBase + i, PU_CACHE)));
				V_DrawFuzzPatch(xpos, 18, static_cast<patch_t*>(W_CacheLumpNum(patchFaceDeadBase + i, PU_CACHE)));
			}
			kpos = 86;
			for (j = 0; j < MAXPLAYERS; j++) {
				if (playeringame[j]) {
